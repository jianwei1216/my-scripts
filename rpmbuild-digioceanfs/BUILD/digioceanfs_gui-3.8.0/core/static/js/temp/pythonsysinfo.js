...
function filesystemtable() {
    var html = "";
    html += "        <h2>" + genlang(30, false, '挂载文件系统') + "</h2>\n";
    html += "        <table id=\"filesystemTable\" cellspacing=\"0\">\n";
    html += "          <thead>\n";
    html += "            <tr>\n";
    html += "              <th>" + genlang(31, false, '挂载点') + "</th>\n";
    html += "              <th>" + genlang(34, false, '类型') + "</th>\n";
    html += "              <th>" + genlang(32, false, '分区') + "</th>\n";
    html += "              <th>" + genlang(33, false, '使用率') + "</th>\n";
    html += "              <th class=\"right\">" + genlang(35, true, '可用') + "</th>\n";
    html += "              <th class=\"right\">" + genlang(36, true, '使用中') + "</th>\n";
    html += "              <th class=\"right\">" + genlang(37, true, '大小') + "</th>\n";
    html += "            </tr>\n";
    html += "          </thead>\n";
    html += "          <tfoot>\n";
    ...
    html += "              <td>&nbsp;</td>\n";
    html += "              <td>&nbsp;</td>\n";
    html += "              <td>" + genlang(38, false, '总大小') + "</td>\n";
...
function formatUptime(sec) {
    ...
    if (intDays) {
        txt += intDays.toString() + "&nbsp;" + genlang(48, false, '天') + "&nbsp;";
    }
    if (intHours) {
        txt += intHours.toString() + "&nbsp;" + genlang(49, false, '时') + "&nbsp;";
    }
    return txt + intMin.toString() + "&nbsp;" + genlang(50, false, '分');
}
...
function refreshVitals(xml) {
    ...
    $("Vitals", xml).each(function getVitals(id) {
        ...
        document.title = '系统信息' + " :" + hostname + " (" + ip + ")";
...
function fillCpu(xml, tree, rootposition, collapsed) {
    ...
    $("Hardware CPU CpuCore", xml).each(function getCpuCore(cpuCoreId) {
        ...
        if (!isNaN(speed)) {
            html += "<tr><td style=\"width:50%\">" + genlang(13, true, 'CPU速度') + ":</td><td>" + formatHertz(speed) + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        collapsed.push(cpucoreposition);
        if (!isNaN(cache)) {
            html += "<tr><td style=\"width:50%\">" + genlang(15, true, '快取大小') + ":</td><td>" + formatBytes(cache) + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        if (virt != undefined) {
            html += "<tr><td style=\"width:50%\">" + genlang(94, true, '虚拟化') + ":</td><td>" + virt + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        if (!isNaN(bus)) {
            html += "<tr><td style=\"width:50%\">" + genlang(14, true, '总线速度') + ":</td><td>" + formatHertz(bus) + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        if (!isNaN(bogo)) {
            html += "<tr><td style=\"width:50%\">" + genlang(16, true, '系统Bogomips') + ":</td><td>" + bogo.toString() + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        if (!isNaN(temp)) {
            html += "<tr><td style=\"width:50%\">" + genlang(51, true, 'CPU温度') + ":</td><td>" + formatTemp(temp, xml) + "</td></tr>\n";
            tree.push(cpucoreposition);
        }
        if (!isNaN(load)) {
...
function refreshHardware(xml) {
    ...
    $("#hardware").empty();
    html += "  <h2>" + genlang(10, false, '硬件信息') + "</h2>\n";
    html += "  <table id=\"HardwareTree\" class=\"tablemain\" style=\"width:100%;\">\n";
    html += "   <tbody class=\"tree\">\n";
    
    html += "    <tr><td colspan=\"2\"><b>" + genlang(11, false, '处理器') + "</b></td></tr>\n";
    html += fillCpu(xml, tree, tree.push(0), closed);
    
    html += "    <tr><td colspan=\"2\"><b>" + genlang(17, false, 'PCI设备') + "</b></td></tr>\n";
    index = tree.push(0);
    closed.push(index);
    html += fillHWDevice(xml, 'PCI', tree, index);
    
    html += "    <tr><td colspan=\"2\"><b>" + genlang(18, false, 'IDE设备') + "</b></td></tr>\n";
    index = tree.push(0);
    closed.push(index);
    html += fillHWDevice(xml, 'IDE', tree, index);
    
    html += "    <tr><td colspan=\"2\"><b>" + genlang(19, false, 'SCSI设备') + "</b></td></tr>\n";
    index = tree.push(0);
    closed.push(index);
    html += fillHWDevice(xml, 'SCSI', tree, index);
    
    html += "    <tr><td colspan=\"2\"><b>" + genlang(20, false, 'USB设备') + "</b></td></tr>\n";
    index = tree.push(0);
    closed.push(index);
...
function refreshMemory(xml) {
    ...
    $("#memory").empty();
    html += "  <h2>" + genlang(27, false, '内存使用率') + "</h2>\n";
    html += "  <table id=\"MemoryTree\" class=\"tablemain\" style=\"width:100%;\">\n";
    html += "   <thead>\n";
    html += "    <tr>\n";
    html += "     <th style=\"width:200px;\">" + genlang(34, true, '类型') + "</th>\n";
    html += "     <th style=\"width:285px;\">" + genlang(33, true, '使用率') + "</th>\n";
    html += "     <th class=\"right\" style=\"width:100px;\">" + genlang(35, true, '可用') + "</th>\n";
    html += "     <th class=\"right\" style=\"width:100px;\">" + genlang(36, true, '使用中') + "</th>\n";
    html += "     <th class=\"right\" style=\"width:100px;\">" + genlang(37, true, '大小') + "</th>\n";
    html += "    </tr>\n";
    html += "   </thead>\n";
    html += "   <tbody class=\"tree\">\n";
    ...
    $("Memory", xml).each(function getMemory(id) {
        ...
        html += "<tr><td style=\"width:200px;\">" + genlang(28, false, '物理内存') + "</td><td style=\"width:285px;\">" + createBar(percent) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(free, xml) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(used, xml) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(total, xml) + "</td></tr>";
        ...
        $("Memory Details", xml).each(function getMemorydetails(id) {
            ...
            if (!isNaN(app)) {
                html += "<tr><td style=\"width:184px;\">" + genlang(64, false, '内核和应用') + "</td><td style=\"width:285px;\">" + createBar(appp) + "</td><td class=\"right\" style=\"width:100px;\">&nbsp;</td><td class=\"right\" style=\"width:100px\">" + formatBytes(app, xml) + "</td><td class=\"right\" style=\"width:100px;\">&nbsp;</td></tr>";
                tree.push(memoryindex);
            }
            if (!isNaN(buff)) {
                html += "<tr><td style=\"width:184px;\">" + genlang(65, false, '缓冲') + "</td><td style=\"width:285px\">" + createBar(buffp) + "</td><td class=\"rigth\" style=\"width:100px;\">&nbsp;</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(buff, xml) + "</td><td class=\"right\" style=\"width:100px;\">&nbsp;</td></tr>";
                tree.push(memoryindex);
            }
            if (!isNaN(cached)) {
                html += "<tr><td style=\"width:184px;\">" + genlang(66, false, '快取') + "</td><td style=\"width:285px;\">" + createBar(cachedp) + "</td><td class=\"right\" style=\"width:100px;\">&nbsp;</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(cached, xml) + "</td><td class=\"right\" style=\"width:100px;\">&nbsp;</td></tr>";
                tree.push(memoryindex);
            }
    ...
    $("Memory Swap", xml).each(function getSwap(id) {
        ...
        total = parseInt($(this).attr("Total"), 10);
        percent = parseInt($(this).attr("Percent"), 10);
        html += "<tr><td style=\"width:200px;\">" + genlang(29, false, '硬盘备用') + "</td><td style=\"width:285px;\">" + createBar(percent) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(free, xml) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(used, xml) + "</td><td class=\"right\" style=\"width:100px;\">" + formatBytes(total, xml) + "</td></tr>";
