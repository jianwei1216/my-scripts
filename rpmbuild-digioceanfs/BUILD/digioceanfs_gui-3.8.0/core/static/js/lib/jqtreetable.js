...
(function(jq){
    ...
    var opts = jq.extend({openImg:"",shutImg:"",leafImg:"",lastOpenImg:"",lastShutImg:"",lastLeafImg:"",vertLineImg:"",blankImg:"",collapse:false,column:0,striped:false,highlight:false,state:true,onselect:false,beforeExpand:false},options),
    ...
    buildText = function(parno, preStr){//Recursively build up the text for the images that make it work
        ...
        for (var y=0,yl=mp.length;y<yl;y++){
            ...
            alert('aaaa');
            alert(mapa.length);
            if (mapa[ro] || mapa.length == 1){//It's a parent as well. Build it's string and move on to it's children
                alert(mapa.length);
    ...
    jq(".parimg", this).each(function(i){
        ...
        if (jqt.parents("tr").next().is(".collapsed")){//If the table row directly below is collapsed
            if (typeof opts.beforexpand == "function"){
                flag = 0;
                var dirname = '';
                for(var i=1; i < num+1; i++){
                    dirname += jqt.parent().text();//get entir dirname  
                }
                target = jq(this);
                opts.beforexpand(target, flag, dirname);
                expandKids(num, (jqt.attr("src")==opts.lastShutImg));//Then expand all children not in collarr
		        if(opts.state){
                    creset(num,true);
                }//If state is set, store in cookie
            }
            else if(opts.beforexpand == false){
                expandKids(num, (jqt.attr("src")==opts.lastShutImg));//Then expand all children not in collarr
		        if(opts.state){
                    creset(num,true);
                }//If state is set, store in cookie
    ...
    //add onselect option by ballma 2011-09-24
    jq('#'+tid).click( function( event ) {//This looks after selecting td's
    ...
    //buildTree = function(map, tid, opts, num){
    //buildTree(map,tid,num,opts);
    alert(mapa);
    alert(mapb);
  };
  return this;
