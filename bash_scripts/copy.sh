...
function mycopy()
{
    for i in 1 2 3; do
        ...
        ssh ${ip_pre}${i} "mkdir -p $tmp_dir2; cp -rf ${copy_dir} $tmp_dir2; cd /var/; tar -czvf ${tmp_dir1}.tar.gz $tmp_dir1; scp ${tmp_dir1}.tar.gz ${save_dir}; rm -rf ${tmp_dir1}.tar.gz ${tmp_dir1}" 
        echo
    done
}
case $1 in
