...
function mycopy()
{
    for i in 1 2 3; do
        ...
        echo ssh ${ip_pre}${i} "mkdir -p $tmp_dir2; ps aux|grep digiocean|grep -v grep > ${tmp_dir2}/digioceanfs.ps.aux; cp -rf ${copy_dir} $tmp_dir2; cd /var/; tar -czvf ${tmp_dir1}.tar.gz $tmp_dir1; scp ${tmp_dir1}.tar.gz ${save_dir}; rm -rf ${tmp_dir1}.tar.gz ${tmp_dir1}" 
